modules = {
    application {
        resource url:'js/application.js'
        resource url:'js/chosen/chosen.jquery.js'
        resource url:'js/chosen/chosen.jquery.min.js'
        resource url:'js/chosen/docsupport/prism.js'
      //	resource url:'js/jquery-easyui/jquery.min.js'
        resource url:'js/jquery-easyui/jquery.easyui.min.js'
        resource url:'js/jquery-easyui/themes/bootstrap/easyui.css'
        resource url:'js/jquery-easyui/datagrid-detailview.js'
        

        resource url:'js/numeral/min/numeral.min.js'	
        
    }

    fileUpload{
        resource url:'addPlugins/jQuery-File-Upload/css/jquery.fileupload.css'

        resource url:'addPlugins/jQuery-File-Upload/js/vendor/jquery.ui.widget.js'
        resource url:'addPlugins/jQuery-File-Upload/js/jquery.fileupload.js'
        resource url:'addPlugins/jQuery-File-Upload/js/jquery.iframe-transport.js'

    }

    datatables{
        resource url:'addPlugins/datatables/dataTables.bootstrap.css'
        
        resource url:'addPlugins/datatables/jquery.dataTables.min.js'
        resource url:'addPlugins/datatables/dataTables.bootstrap.min.js'        
    }
}